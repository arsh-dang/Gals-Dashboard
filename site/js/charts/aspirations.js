...
(function () {
  ...
  function render(container, rows, meta, colors) {
    ...
    const y = d3.scaleBand().domain(items).range([margin.top, height - margin.bottom]).paddingInner(0.25);
    const sub = d3.scalePoint().domain(['gals', 'nonGals']).range([-y.bandwidth() / 2 + 8, y.bandwidth() / 2 - 8]);
    ...
    items.forEach((item, i) => {
      ...
      svg.append('rect')
        ...
        .attr('y', y(item)).attr('height', y.bandwidth())
    ...
    function drawGroup(item, key, label, color) {
      ...
      const cy = y(item) + y.bandwidth() / 2 + sub(key);
    ...
    container.querySelector('svg').style.minWidth = '720px';
  }
