...
import sitdb
from conftest import db as database  # noqa: F401  (the SQLite/PostgreSQL fixture)

IntegrityError = sitdb.IntegrityError


@pytest.fixture()
def db(database):  # noqa: F811
    con = sitdb.connect(database)  # creates the tables from schema.sql
    con.execute("INSERT INTO survey_version VALUES ('v3', 'test')")
    for wave in ("w1", "w2"):
        con.execute(
            "INSERT INTO survey_wave VALUES (?, 'v3', NULL, NULL, 'x.csv', '2026-01-01T00:00:00', ?, NULL)",
            (wave, True),
        )
    yield con
    con.close()
...
def test_wave_needs_a_known_survey_version(db):
    with pytest.raises(IntegrityError):
        db.execute("INSERT INTO survey_wave VALUES ('w3', 'v9', NULL, NULL, 'x.csv', '2026', TRUE, NULL)")
